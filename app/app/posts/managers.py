...
class PostManagerQueryset(models.QuerySet):
    def with_comments(self, pk):
        ...
        return self.prefetch_related(Prefetch('comments', queryset=Comment.objects.filter(id__in=Subquery(latest_comments)).order_by('-created_at')))
